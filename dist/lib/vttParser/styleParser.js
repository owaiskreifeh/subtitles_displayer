...
function getAbsoluteLengthInPixels(value, containerHeight) {
  return containerHeight * value / 50
  /** cell/line height */
  + 'px';
}

function convertLengthValue(lengthValue, containerHeight) {
  const lengthValueInfo = getLengthValueInfo(lengthValue);

  if (!lengthValueInfo) {
    return lengthValue;
  }

  const unit = lengthValueInfo.unit,
        value = lengthValueInfo.value;

  switch (unit) {
    case '%':
      return getAbsoluteLengthInPixels(value / 100, containerHeight);

    case 'c':
      return getAbsoluteLengthInPixels(value, containerHeight);
